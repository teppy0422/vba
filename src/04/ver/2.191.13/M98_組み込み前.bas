...
Dim 徳島sheet As String: 徳島sheet = "PVSW (2)"
...
With Workbooks(徳島book).Sheets(徳島sheet) '1
...
With Workbooks(徳島book).Sheets(徳島sheet)
                ...
                With Workbooks(生準book).Sheets(生準sheet)
                    ...
                    For Y2 = myKey.Row + 1 To 生準lastrow
                        ...
                        If 構成 = 構成2 Then
                            ...
                            Next i
                            If myCount >= 2 Then
                                 Workbooks(徳島book).Sheets(徳島sheet).Cells(Y, X).Interior.color = 16764159
                            End If
                            GoTo line10
                        End If
line05:
...
Public Function 竿レイアウト図の作成ver2179(CB0, 型式str, 号機str)
    ...
    '出力先bookを作成
    Workbooks.Open アドレス(0) & "\genshi\原紙_竿レイアウト.xlsm"
        ...
        For X = myKey.Column + 1 To lastCol
            ...
            If InStr(配置端末, "_" & 端末str & "_") = 0 Then
                If 回路符号.Font.color = 5287936 Then
                    ...
                    End Select
                    画像URL = アドレス(1) & "\部材一覧+_略図\" & 矢崎str & "_1_001.emf"
...
'                Exit For
    ...
    'onkeyを渡す
    On Error Resume Next
        'WB(1).VBProject.VBComponents(WS(1).CodeName).CodeModule.AddFromFile アドレス(0) & "\OnKey" & "\003_竿レイアウト.txt"
    On Error GoTo 0
    ws(1).Activate
    wb(1).Save
    '設定ファイルを作成

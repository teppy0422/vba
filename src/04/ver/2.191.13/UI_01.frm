...
Private Sub B0_Click()
    ...
    cbx5.Value = False
    cbxQR.Value = False
    'PIC00.Picture = LoadPicture(アドレス(0) & "\ハメ図sample_" & "4511000000" & ".jpg")
End Sub
...
Public Function CB選択変更()
    ...
    Else
        ...
        PIC00.Picture = LoadPicture(アドレス(0) & "\menu\" & 状態 & ".bmp")
        On Error GoTo 0
    End If
...
Public Function マジック選択変更()
    ...
    PIC01.Picture = LoadPicture(アドレス(0) & "\" & 状態 & ".jpg")
End Function
...
Private Sub CommandButton1_Click()
    ...
    If 後ハメ作業者 = True Then
        If 製品品番RANc = 1 Then
            アドレス(3) = 製品品番RAN(製品品番RAN_read(製品品番RAN, "後ハメ作業者取得"), 1)
            If Dir(アドレス(3)) <> "" Then
                Set wb(3) = Workbooks.Open(fileName:=アドレス(3), UpdateLinks:=0, ReadOnly:=True, IgnoreReadOnlyRecommended:=True)
    ...
    If サンプル作成モード = True Then
                        ...
                        For i4 = 0 To CB4.ListCount - 1
                            For i8 = 0 To CB8.ListCount - 1
                                ...
                                For ii = 0 To 1
                                    ...
                                    Call ハメ図作成_Ver2001(cb選択, cb5str(ii), cb6str(ii))
        ...
        Next i0
    Else
        Call ハメ図作成_Ver2001(cb選択, CB5.Value, CB6.Value)
    End If
    ...
    If cbx0.Value = True Then
        msg = サブ図作成_Ver2023(CB6.Value)
        If msg <> "" Then
            DoEvents
            a = MsgBox("次の端末が[端末一覧]にあって[PVSW_RLTF]にありません." & vbCrLf & vbCrLf & msg, , "サブ図の作成エラー")
        End If
    End If
    ...
    End If
    'Call 完了しました(myBook)
    'mybook.VBProject.VBComponents(Sheets("ハメ図_" & CB5.Value & Replace(CB6.Value, " ", "")).CodeName).CodeModule.AddFromFile アドレス(0) & "\002_問連書作成_マルマ.txt"
End Sub
...
Private Sub UserForm_Initialize()
    ...
    Dim 項目(10) As String
    項目(0) = "図を作成しない,電線サイズのみ,ポイント,回路符号,構成,相手端末"

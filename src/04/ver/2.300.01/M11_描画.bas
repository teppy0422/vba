...
Public Function ColorMark3(端末, xLeft As Single, yTop As Single, myWidth As Single, myHeight As Single, 色呼, 種類, 形状, マルマ1, シールドフラグ, 選択出力, サイズ呼, ハメ, EmptyPlug, PlugColor, RowStr, info)
'    Dim 端末 As String
'    Dim xLeft As Single
'    Dim yTop As Single
...
'    dim
    ...
    'Dim myLastRo
        ...
        If BufMarkStr <> "" Then
            If 後ハメ点滅 = True Then
                lower = searchRan_ver4(後ハメ作業者ran, infoSP(0), "構成", "作業順", 0)
                If lower = "False" Then lower = " "
            Else
                upper = CStr(選択出力)
                lower = CStr(サイズ呼)
            End If
            ...
            End If
            If 形状 = "Box" Then BufRes = BoxFill(xLeft, yTop, myWidth, myHeight, BufClo1, gyo, BufClo2, マルマ1, CStr(シールドフラグ), upper, lower, CStr(ハメ))
            If 形状 = "Cir" Then BufRes = CircleFill(gyo, xLeft, yTop, myWidth, myHeight, BufClo1, BufClo2, CStr(マルマ1), CStr(シールドフラグ), upper, lower, CStr(ハメ))
            ...
            End If
        Else
            If 形状 = "Box" Then BufRes = BoxNull(xLeft, yTop, myWidth, myHeight, upper, lower, CStr(EmptyPlug), CStr(PlugColor))
            If 形状 = "Cir" Then BufRes = CircleNull(xLeft, yTop, myWidth, myHeight, upper, lower, CStr(EmptyPlug), CStr(PlugColor))
        End If
...
Public Function create_Shape(ByVal shapeType As Long, ByVal mytext As String, ByVal color As String, ByVal position As String, ByVal size As String)
     
    Dim ob As Object, colorSp As Variant, sizeSp As Variant, positionSp As Variant, maxWidthPoint As Long
    maxWidthPoint = 40
    colorSp = Split(color, ","): sizeSp = Split(size, ","): positionSp = Split(position, ",")
    Set ob = ActiveSheet.Shapes.AddShape(shapeType, positionSp(0), positionSp(1), sizeSp(0), sizeSp(1))
    
    With ob
        .Fill.ForeColor.RGB = CLng(colorSp(0))
        With .Line
            .Visible = msoTrue
            .Weight = 1.75
        End With
        ...
        With .TextFrame2
            ...
            .WordWrap = msoFalse
            .TextRange.Font.size = maxWidthPoint / Len(mytext)
            .TextRange.Font.Name = "HGｺﾞｼｯｸE"
            .TextRange.Text = mytext
            .TextRange.Font.Fill.ForeColor.RGB = CLng(colorSp(1))
...
Public Function Tube_convert(ByVal beforeText As String) As Variant
    Select Case beforeText
        Case "GRO", "HCOT-FR"
    End Select
End Function

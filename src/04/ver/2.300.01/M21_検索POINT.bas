...
Public Function POINTset(book, sheet)
    Dim maxRow As Long, maxCol As Long, row_ As Long
    Dim pointTitle As Range
    With Workbooks(book).Sheets(sheet)
        row_ = .Cells.Find("端末矢崎品番", , , 1).Row
        maxCol = .Cells(row_, .Columns.count).End(xlToLeft).Column
        Set pointTitle = .Range(.Cells(row_, 1), .Cells(row_, maxCol))
